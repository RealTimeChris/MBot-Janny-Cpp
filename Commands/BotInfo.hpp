	  ...
	  public:
		...
		virtual void execute(BaseFunctionArguments& newArgs) {
			try {
				...
				messageEmbed.setImage(newArgs.discordCoreClient->getBotUser().avatar);
				messageEmbed.setColor("FEFEFE");
				messageEmbed.setTitle("__**Bot Info:**__");
				messageEmbed.setTimeStamp(getTimeAndDate());
				messageEmbed.addField(
					"__Bot Name:__", newArgs.discordCoreClient->getBotUser().userName + "#" + newArgs.discordCoreClient->getBotUser().discriminator, true);
				messageEmbed.addField("__Bot ID:__", newArgs.discordCoreClient->getBotUser().id, true);
				messageEmbed.addField("__Guild Count:__", std::to_string(guilds.size()), true);
				messageEmbed.addField("__Created At:__", newArgs.discordCoreClient->getBotUser().createdAt, true);
