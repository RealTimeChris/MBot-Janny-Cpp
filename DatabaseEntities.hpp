	...
	class DatabaseManagerAgent {
	  ...
	  protected:
		...
		static DiscordUserData parseUserData(bsoncxx::document::value docValue) {
			DiscordUserData userData{};
			try {
				userData.userName		= docValue.view()["userName"].get_string().value;
		...
		static DiscordGuildData parseGuildData(bsoncxx::document::value docValue) {
			...
			try {
				...
				guildData.borderColor	   = docValue.view()["borderColor"].get_string().value;
				guildData.vanityInviteUses = docValue.view()["vanityInviteUses"].get_int32().value;
				guildData.guildName		   = docValue.view()["guildName"].get_string().value;
				...
				for (auto& value: docValue.view()["userBanInfo"].get_array().value) {
					...
					for (auto& value2: value["userBans"].get_array().value) {
						...
						newData02.userName	= value2["userName"].get_string().value;
						newData02.avatarUrl = value2["avatarUrl"].get_string().value;
						newData02.bannedAt	= value2["bannedAt"].get_string().value;
						newData02.reason	= value2["reason"].get_string().value;
				...
				guildData.roleManager.message	= roleManager.view()["message"].get_string().value;
				...
				for (auto& value: docValue.view()["logs"].get_array().value) {
					...
					newData.loggingChannelName = value.get_document().value["loggingChannelName"].get_string().value;
					newData.name			   = value.get_document().value["name"].get_string().value;
					newData.nameSmall		   = value.get_document().value["nameSmall"].get_string().value;
		...
		static DiscordGuildMemberData parseGuildMemberData(bsoncxx::document::value docValue) {
			DiscordGuildMemberData guildMemberData{};
			try {
				guildMemberData.guildMemberMention = docValue.view()["guildMemberMention"].get_string().value;
				guildMemberData.guildId			   = static_cast<uint64_t>(docValue.view()["guildId"].get_int64().value);
				guildMemberData.displayName		   = docValue.view()["displayName"].get_string().value;
				guildMemberData.globalId		   = docValue.view()["globalId"].get_string().value;
				guildMemberData.guildMemberId	   = static_cast<uint64_t>(docValue.view()["guildMemberId"].get_int64().value);
				guildMemberData.userName		   = docValue.view()["userName"].get_string().value;
				...
				for (auto& value: docValue.view()["previousPermissionOverwrites"].get_array().value) {
					...
					newOverWriteData.id	   = std::stoull(jsonifier::string{ value.get_document().view()["id"].get_string().value }.data());
				...
				for (auto& value: docValue.view()["invites"].get_array().value) {
					...
					newData.inviteCode	= value.get_document().view()["inviteCode"].get_string().value;
